...
typedef ViewportBuilder = Widget Function(BuildContext context, Matrix4 matrix, Rect visibleRect);
typedef OnSelectionUpdate = void Function(Rect sectionRect, List<Offset> points);

mixin TransformationControllerMixin on TransformationController {
  toView(Rect rect) {}
}
...
class InteractiveViewport extends StatefulWidget {
  ...
  InteractiveViewport({
    ...
    this.enableFling = false,
        ...
        assert((boundaryMargin.horizontal.isInfinite && boundaryMargin.vertical.isInfinite) ||
            (boundaryMargin.top.isFinite && boundaryMargin.right.isFinite && boundaryMargin.bottom.isFinite && boundaryMargin.left.isFinite)),
        super(key: key) {}

  final OnSelectionUpdate? onSelectionUpdate;
  final VoidCallback? onSelectionEnd;
  final ValueChanged<PointerHoverEvent>? onTap;
  final ValueChanged<PointerDownEvent>? onPointerDown;
  final ValueChanged<PointerHoverEvent>? onSecondaryTap;
  ...
  static Vector3? getNearestPointInside(Vector3 point, Quad quad) {
    ...
    final List<Vector3> closestPoints = <Vector3>[
      InteractiveViewport.getNearestPointOnLine(point, quad.point0, quad.point1),
      InteractiveViewport.getNearestPointOnLine(point, quad.point1, quad.point2),
      InteractiveViewport.getNearestPointOnLine(point, quad.point2, quad.point3),
      InteractiveViewport.getNearestPointOnLine(point, quad.point3, quad.point0),
    ];
  ...
  @override
  InteractiveViewerState createState() => InteractiveViewerState();
}

class InteractiveViewerState extends State<InteractiveViewport> with TickerProviderStateMixin {
  TransformationController? _transformationController;
  ...
  Rect get _boundaryRect {
    ...
    assert(boundaryRect.isFinite || (boundaryRect.left.isInfinite && boundaryRect.top.isInfinite && boundaryRect.right.isInfinite && boundaryRect.bottom.isInfinite),
  ...
  Matrix4 _matrixTranslate(Matrix4 matrix, Offset translation) {
    ...
    final Offset alignedTranslation = widget.alignPanAxis && _panAxis != null ? _alignAxis(translation, _panAxis!) : translation;
  ...
  void scaleStart(ScaleStartDetails details) => _onScaleStart(details);

  void scaleUpdate(ScaleUpdateDetails details) => _onScaleUpdate(details);

  void scaleEnd(ScaleEndDetails details) => _onScaleEnd(details);

  void doubleTap() => _onDoubleTap();
  ...
  void _onScaleUpdate(ScaleUpdateDetails details) {
    ...
    if (!_secondaryButton && widget.selectionMode && details.scale == 1.0) {
      _gestureType = _GestureType.selection;
    } else if (_gestureType == _GestureType.pan) {
    ...
    switch (_gestureType!) {
      ...
      case _GestureType.pan:
        ...
        break;
      case _GestureType.selection:
        drawPathPoints.add(details.localFocalPoint);
        var _selectRect = Rect.fromPoints(_interactionStart!, details.localFocalPoint);
        widget.onSelectionUpdate?.call(_selectRect, List.from(drawPathPoints));
        return;
    }
  ...
  void _onScaleEnd(ScaleEndDetails details) {
    ...
    if (widget.selectionMode) {
      if (drawPathPoints.length > 0) widget.onSelectionEnd?.call();
      drawPathPoints.clear();
      return;
    }
  ...
  void _receivedPointerSignal(PointerSignalEvent event) {
    if (widget.mouseWheelEnabled && event is PointerScrollEvent) {
      ...
      _debounce?.run(_finishMouseScroll, milliseconds: 100);
    }
  }
  ...
  void initState() {
    ...
    _matrixController = AnimationController(vsync: this, duration: Duration(milliseconds: 800));
    _debounce = Debounce(milliseconds: 100);
  }
  ...
  Widget build(BuildContext context) {
    ...
    Widget child = KeyedSubtree(
      key: _childKey,
      child: widget.child ?? widget.builder!.call(context, _transformationController!.value, rect),
    );
    ...
    return Listener(
      key: _parentKey,
      onPointerHover: _onPointHover,
      onPointerDown: (event) {
        _secondaryButton = event.buttons == kSecondaryMouseButton;
        widget.onPointerDown?.call(event);
      },
      onPointerSignal: _receivedPointerSignal, //mouse scroll event
      ...
      child: GestureDetector(
        ...
        onSecondaryTap: () => widget.onSecondaryTap?.call(_hoverPoint!),
        onTap: () => widget.onTap?.call(_hoverPoint!),
        child: child,
      ),
    );
  }
  ...
  ///
  /// zoom to global view rect
  /// [rect] rect in global (original scale)
  ///
  void zoomToRect(Rect rect) {
    Matrix4 endMatrix;
    double scale = _transformationController!.value.getMaxScaleOnAxis();

    final double desiredScale = _viewport.width / rect.width;
    final double scaleChange = desiredScale / scale;
    Offset focalPointScene = _transformationController!.toScene(rect.topLeft);

    var translation = _transformationController!.value.getTranslation();
    focalPointScene = Offset(translation.x, translation.y);

    endMatrix = _matrixScale(_transformationController!.value, scaleChange);

    print('scale change: ${scaleChange}, point scene: ${focalPointScene}');
    // After scaling, translate such that the event's position is at the
    // same scene point before and after the scale.

    final Offset focalPointSceneScaled = rect.topLeft.transformInvert(endMatrix); // _transformationController.toScene(rect.center);
    print('focalPointSceneScaled: ${focalPointSceneScaled}');
    endMatrix = _matrixTranslate(endMatrix, focalPointSceneScaled - focalPointScene);

    Matrix4Tween tween = Matrix4Tween(begin: _transformationController!.value, end: endMatrix);
    _matrixAnimate(tween);
  }

  void _matrixAnimate(Matrix4Tween animation) {
    if (_matrixController!.isAnimating) {
      print('matrix animating');
      return;
    }
    _matrixAnimation?.removeListener(_matrixAnimationUpdate);
    _matrixAnimation = null;
    _matrixController!.reset();

    _matrixAnimation = animation.animate(CurvedAnimation(
    ...
    widget.onInteractionStart?.call(ScaleStartDetails(localFocalPoint: _viewport.center));
  ...
  _matrixAnimationUpdate() {
    ...
    widget.onInteractionUpdate?.call(ScaleUpdateDetails(
      localFocalPoint: _viewport.center,
      focalPoint: _viewport.center,
    ));
  }

  void _onPointHover(PointerHoverEvent event) {
    _hoverPoint = event;
    widget.onPointerHover?.call(event);
  }

  void _scaleAnimateUpdate() {
    _onScaleUpdate(ScaleUpdateDetails(
      localFocalPoint: _scaleByController ? _viewport.center : _hoverPoint?.localPosition,
      focalPoint: _scaleByController ? _viewport.center : _hoverPoint!.position,
      scale: _scaleAnimation!.value,
    ));
  }

  void _onDoubleTap() {
    _scaleByController = false;
    _scale(.5);
  }

  bool _scaleByController = false;

  void zoomIn() {
    if (_transformationController!.value.getMaxScaleOnAxis() >= widget.maxScale) return;
    _scaleByController = true;
    _scale(0.2);
  }

  void zoomOut() {
    if (_transformationController!.value.getMaxScaleOnAxis() <= widget.minScale) return;
    _scaleByController = true;
    _scale(-0.2);
  }

  void scaleWithAnimation(Tween<double> animation) {
    if (_scaleController!.isAnimating) {
      return;
    }
    _scaleAnimation?.removeListener(_scaleAnimateUpdate);
    _scaleAnimation = null;
    _scaleController!.reset();

    _scaleAnimation = animation.animate(CurvedAnimation(
      parent: _scaleController!,
      curve: Curves.decelerate,
    ));
    _scaleAnimation!
      ..addListener(_scaleAnimateUpdate)
      ..addStatusListener((status) {
        if (status == AnimationStatus.completed) {
          _onScaleEnd(ScaleEndDetails(velocity: Velocity.zero));
        }
      });
    _onScaleStart(ScaleStartDetails(localFocalPoint: _scaleByController ? _viewport.center : _hoverPoint!.localPosition, focalPoint: _scaleByController ? _viewport.center : _hoverPoint!.position));
    _scaleController!.forward();
  }

  void _scale(double scale) {
    scaleWithAnimation(Tween<double>(begin: 1.0, end: 1.0 + scale));
  }
...
Quad _getAxisAlignedBoundingBoxWithRotation(Rect rect, double rotation) {
  ...
  return InteractiveViewport.getAxisAlignedBoundingBox(boundariesRotated);
}
...
Offset _exceedsBy(Quad boundary, Quad viewport) {
  ...
  for (final Vector3 point in viewportPoints) {
    final Vector3 pointInside = InteractiveViewport.getNearestPointInside(point, boundary)!;
