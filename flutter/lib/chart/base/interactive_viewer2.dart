...
class InteractiveViewer2 extends StatefulWidget {
  ...
  InteractiveViewer2({
    ...
    required Widget this.child,
        ...
        // a mix of both.
        assert(
          (boundaryMargin.horizontal.isInfinite && boundaryMargin.vertical.isInfinite) || (boundaryMargin.top.isFinite && boundaryMargin.right.isFinite && boundaryMargin.bottom.isFinite && boundaryMargin.left.isFinite),
        ),
        builder = null;
  ...
  @override
  State<InteractiveViewer2> createState() => InteractiveViewerState2();
}

class InteractiveViewerState2 extends State<InteractiveViewer2> with TickerProviderStateMixin {
  ...
  Offset? _referenceLocalFocalPoint; // Point where the current gesture began.
  ...
  void _onScaleStart(ScaleStartDetails details) {
    ...
    _referenceLocalFocalPoint = details.localFocalPoint;
  ...
  void _onScaleUpdate(ScaleUpdateDetails details) {
    ...
    switch (_gestureType!) {
      ...
      case _GestureType.pan:
        ...
        final Offset localDelta = details.localFocalPoint - _referenceLocalFocalPoint!;

        // print('$translationChange -> $localDelta');
  ...
  void _onScaleEnd(ScaleEndDetails details) {
    ...
    _referenceLocalFocalPoint = null;
    ...
    if (_gestureType == _GestureType.pan) {
      ...
      final double scale = _transformationController!.value.getMaxScaleOnAxis();
      final FrictionSimulation frictionSimulation = FrictionSimulation(widget.interactionEndFrictionCoefficient * widget.scaleFactor, scale, details.scaleVelocity / 10);
      ...
      _scaleAnimation = Tween<double>(begin: scale, end: frictionSimulation.x(tFinal)).animate(CurvedAnimation(parent: _scaleController, curve: Curves.decelerate));
  ...
  void _receivedPointerSignal(PointerSignalEvent event) {
    ...
    if (event is PointerScrollEvent) {
      if (event.kind == PointerDeviceKind.trackpad) {
        ...
        _transformationController!.value = _matrixTranslate(_transformationController!.value, (newFocalPointScene - focalPointScene));

        widget.onInteractionUpdate?.call(ScaleUpdateDetails(focalPoint: event.position - event.scrollDelta, localFocalPoint: event.localPosition - localDelta, focalPointDelta: -localDelta));
        widget.onInteractionEnd?.call(ScaleEndDetails());
        return;
      }
  ...
  @override
  void didUpdateWidget(InteractiveViewer2 oldWidget) {
  ...
  void _scaleAnimateUpdate() {
    ...
    _onScaleUpdate(ScaleUpdateDetails(
      localFocalPoint: _scaleByController ? _viewport.center : _hoverPoint,
      focalPoint: _scaleByController ? _viewport.center : _hoverPoint!,
  ...
  void scaleWithAnimation(Tween<double> animation, [Offset? anchor]) {
    if (_scaleController.isAnimating) {
      return;
    }
    ...
    _onScaleStart(ScaleStartDetails(localFocalPoint: _scaleByController ? _viewport.center : _hoverPoint, focalPoint: _scaleByController ? _viewport.center : _hoverPoint!));
    _scaleController.forward();
  }

  void _scale(double scale, [Offset? anchor]) {
...
class TransformationController2 extends ValueNotifier<Matrix4> {
  /// Create an instance of [TransformationController].
  ///
  /// The [value] defaults to the identity matrix, which corresponds to no
  /// transformation.
  TransformationController2([Matrix4? value]) : super(value ?? Matrix4.identity());
...
Quad _getAxisAlignedBoundingBoxWithRotation(Rect rect, double rotation) {
  ...
  return InteractiveViewer2.getAxisAlignedBoundingBox(boundariesRotated);
}
