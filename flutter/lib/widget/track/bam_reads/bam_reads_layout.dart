...
class BamReadsLayout extends TrackLayout {
...
//      return _featureRowMap[a.id] == null ? 1 : -1;
    ...
    for (int i = 0; i < features.length; i++) {
      feature = features[i]..linkTo = null;
      feature.index = i;

      Range range = feature.range;

      if (!visibleRange.collide(range)) continue;
...
//      }
      ...
      _featureRow = featureRowMap[feature.uniqueId] ?? _findProperPositionRow(feature.uniqueId, start, _blockEnd, fHeight, rowHeight, rowSpace, padding, _childrenCount, _rowFeaturePositionList);
    ...
    double _maxHeight = _rowFeaturePositionList.values.map((e) => _findMaxHeight(e, _horizontal)).max() ?? 60.0; //.reduce((a, b) => a > b ? a : b);
    maxHeight = _maxHeight;
//    print(_featureRowMap);
  }

  void calculateGap(BamReadsFeature? feature) {
    if (feature == null) return;
    List<Feature>? gaps = feature.gaps;
    if (gaps != null && gaps.length > 0) {
      //break feature by gaps
      List<Rect> breakRects = [];
      double left, right;
      for (int i = 0; i < gaps.length; i++) {
        left = i == 0 ? feature.rect!.left : gaps[i - 1].rect!.right;
        right = gaps[i].rect!.left;
        breakRects.add(Rect.fromLTRB(left, feature.rect!.top, right, feature.rect!.bottom));
        if (i == gaps.length - 1) {
          left = gaps[i].rect!.right;
          right = feature.rect!.right;
          breakRects.add(Rect.fromLTRB(left, feature.rect!.top, right, feature.rect!.bottom));
        }
      }
      feature.rects = breakRects;
    } else {
      feature.rects = null;
    }
  }
  ...
  double _measureBlockMaxEnd(List<Feature> features, Scale<num, num> scale, double labelFontSize) {
    return features.map((f) {
      ...
      double textWidth = measureTextWidth(maxLabel, labelFontSize);
  ...
  int _findProperPositionRow(id2, start, end, height, rowHeight, rowSpace, EdgeInsets padding, childrenCount, Map<int, List<FeaturePosition>> rowFeatureList) {
...
//      }
  ...
  int _findMaxRow(double start, Iterable<FeaturePosition> blockPositions) {
    ...
    FeaturePosition _position = _blockPositions.maxBy((e) => e.row)!; // _blockPositions.reduce((value, element) => value.row > element.row ? value : element);
    return _position.row;
  }

  double _findMaxHeight(Iterable<FeaturePosition> blockPositions, bool _horizontal) {
    if (_horizontal) {
      return blockPositions.map((e) => e.rect.bottom).max() ?? 0;
      // return blockPositions.reduce((value, element) => value.rect.bottom > element.rect.bottom ? value : element).rect.bottom;
    } else {
      return blockPositions.map((e) => e.rect.right).max() ?? 0;
