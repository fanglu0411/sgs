...
class MonotoneX {
  ...
  static double slope3(double x0, double y0, double x1, double y1, double x2, double y2) {
    double h0 = x1 - x0;
    double h1 = x2 - x1;
  ...
  static Path point(Path path, double x0, double y0, double x1, double y1, double t0, double t1) {
    var dx = (x1 - x0) / 3;
  ...
  static Path smoothPath(List<Offset> points) {
    ...
    for (int i = 0; i < targetPoints.length; i++) {
      var t1;
      var x = targetPoints[i].dx;
      var y = targetPoints[i].dy;
      if (x == x1 && y == y1) break;
      switch (i) {
        case 0:
          path.moveTo(x, y);
        ...
        case 2:
          t1 = slope3(x0, y0, x1, y1, x, y);
          point(path, x0, y0, x1, y1, slope2(x0, y0, x1, y1, t1), t1);
          break;
        default:
          t1 = slope3(x0, y0, x1, y1, x, y);
          point(path, x0, y0, x1, y1, t0, t1);
      }
      x0 = x1;
      y0 = y1;
      x1 = x;
